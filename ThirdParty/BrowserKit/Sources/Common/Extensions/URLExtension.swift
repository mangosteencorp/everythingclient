...
extension URL {
    ...
    public var shortDisplayString: String {
        guard let publicSuffix = publicSuffix, let baseDomain = normalizedHost else {
            return normalizedHost ?? absoluteString
        }
    ...
    public var domainURL: URL {
        if let normalized = normalizedHost {
    ...
    public var decodeReaderModeURL: URL? {
        if isReaderModeURL || isSyncedReaderModeURL {
...
private struct ETLDEntry: CustomStringConvertible {
    ...
    init(entry: String) {
        self.entry = entry
        isWild = entry.hasPrefix("*")
        isException = entry.hasPrefix("!")
    }
